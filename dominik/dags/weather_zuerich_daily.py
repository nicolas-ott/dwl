...
def fetchUrlList():
    ...
    df_sources.to_sql('weather_zuerich_daily_urls', con= dbconnect, if_exists='replace',
          index=False)
...
def fetchAllCsv():
    ...
    # write df to db
    df.to_sql('weather_zuerich_daily', con= dbconnect, if_exists='replace',
          index=False)
    dbconnect = pg.connect(conn_string
                        )
    dbconnect.autocommit = True
